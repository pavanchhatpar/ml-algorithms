...
data_root = "../../../../Projects/MS Projects/ML/trec8/vol4/fr94"

filenames=[]
for (dirpath, dirnames, fnames) in os.walk(data_root):
    filenames.extend(map(lambda fname: os.path.join(dirpath,fname), fnames))
...
def parse_docs():
    for file in filenames:
        print(file)
        try:
            soup = bsoup(open(file), 'html.parser')
        except UnicodeDecodeError:
            soup = bsoup(open(file, encoding='ISO-8859-15'), 'html.parser')
